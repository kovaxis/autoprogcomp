...
def main():
    ...
    in_values = sheet.values().get(spreadsheetId=config.spreadsheet_id, range=config.sheet_name).execute()
    in_mat = in_values.get("values", [])

    # Show "calculating..."
    sheet.values().update(
        spreadsheetId=config.spreadsheet_id,
        range=f"{config.sheet_name}!A1:A1",
        body={"values": [["Updating..."]]},
    ...
    sheet.values().update(
        spreadsheetId=config.spreadsheet_id,
        range=f"{config.sheet_name}!A1:A1",
        body={"values": [[f"Updated at {updtime}"]]},
        valueInputOption="RAW",
    )
